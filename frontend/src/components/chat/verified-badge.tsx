...
export type VerifiedBadgeVariant = 'standard' | 'pro' | 'geek'

export function hasPremiumPlan(sub_plan?: string | null) {
  return sub_plan === 'PRO' || sub_plan === 'GEEK'
}

export function getVerifiedBadgeVariant(sub_plan?: string | null): VerifiedBadgeVariant {
  if (sub_plan === 'GEEK') return 'geek'
  if (sub_plan === 'PRO') return 'pro'
  return 'standard'
}
...
export function isVerifiedAccountIdentity(
  ...
  if (identity.isVerified || identity.is_verified || hasPremiumPlan(identity.sub_plan)) return true

  const currentIsVerified =
    Boolean(currentIdentity?.isVerified || currentIdentity?.is_verified) ||
    hasPremiumPlan(currentIdentity?.sub_plan)
...
export function VerifiedBadge({
  ...
  return (
    <span
      ...
      title={label}
      aria-label={variant === 'standard' ? label : `${label} subscriber`}
    >
      <svg viewBox="0 0 24 24" fill="none" aria-hidden="true" className="block h-full w-full">
        <path
          ...
          strokeWidth="1.5"
        />
        <path d="M9 12L11 14L15 10" stroke="#FFFFFF" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </span>
  )
}
